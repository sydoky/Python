class TicTacToe:
    def __init__(self):
        ...
        self.board_coordinates = Board(3, 3)
...
class Board:
    ...
    def numerate(self):
        for row in range(self.rows):
            for colon in range(self.colons):
                self.fields.append(Sign(str(row * self.colons + colon + 1), row, colon))
    ...
    def three_in_line(self):  # figure out if we have three Xs or Os in row.
        for n in range(self.rows):
            if self.fields[n * self.colons].character == self.fields[n * self.colons + 1].character == self.fields[n * self.colons + 2].character != "_":
                return self.fields[n * self.colons]
            if self.fields[n].character == self.fields[n + self.colons].character == self.fields[n + self.colons * 2].character != "_":
